...
local index = 1
local twirlNotify = false
 
local StartKey = 0x3C3DD371 -- Page Down
local Prompts = {
    Do = 0xDEB34313,    -- right
    End = 0xA65EBAB4,   -- left
    Prev = 0x05CA7C52,  -- down
    Next = 0x6319DB71,  -- up
}
...
AddEventHandler('ricx_guntwirl:toggleTwirl', function()
    tricking = not tricking
    if tricking and not twirlNotify then
        exports.ox_lib:notify({
            title = 'GunTwirl Hotkey',
            description = 'Press PAGE DOWN to also open it.',
            type = 'info'
        })
        twirlNotify = true
    end
